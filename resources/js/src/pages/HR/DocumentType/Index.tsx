            ...
            {loading ? (
                ...
                <div className="table-responsive bg-white dark:bg-black rounded-lg shadow-sm border border-gray-100 dark:border-gray-800">
                    <table className="table-hover table-striped w-full table">
                        ...
                        <tbody>
                            {paginatedDocumentTypes.map((type: any, index: number) => (
                                <tr key={type.id}>
                ...
                >
                    <div className="flex flex-col h-full rounded-xl overflow-hidden
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5 font-medium">
                                        {editingDocumentType ? t('update_document_type_detail_desc') : t('fill_document_type_detail_desc')}
                                    </p>
                                </div>
                            </div>
                        ...
                        <PerfectScrollbar options={{ suppressScrollX: true }} className="flex-1 min-h-0">
                            <form id="document-type-form" onSubmit={handleSubmit} className="p-6 space-y-8">
                                
                                {/* Block 1: Document Identity */}
                                <section className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 rounded-lg bg-primary/10 text-primary">
                                            <IconUserSquareRounded size={18} />
                                        </div>
                                        <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest">{t('document_identity_title', 'Document Identity')}</h3>
                                    </div>

                                    <div className="bg-slate-50/50 dark:bg-slate-800/20 p-5 rounded-xl border border-slate-100 dark:border-slate-800/50">
                                        <div className="space-y-1.5">
                                            <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">{t('document_type_name_label')} <span className="text-rose-500">*</span></Label>
                                            <Input
                                                name="name"
                                                value={formData.name}
                                    ...
                                    <div className="bg-slate-50/50 dark:bg-slate-800/20 p-5 rounded-xl border border-slate-100 dark:border-slate-800/50">
                                        <div className="space-y-1.5">
                                            ...
                                            <Textarea
                                                ...
                                                onChange={handleChange}
                                                placeholder={t('document_type_desc_placeholder')}
                                                rows={4}
                                    ...
                                    <div className="bg-slate-50/50 dark:bg-slate-800/20 p-5 rounded-xl border border-slate-100 dark:border-slate-800/50 space-y-4">
                                        <div className="flex items-center justify-between p-2">
                                            <div className="space-y-0.5">
                                                <Label className="text-sm font-bold text-slate-700 dark:text-slate-200">{t('required_document_label')}</Label>
                                                <p className="text-xs text-slate-500">{t('document_required_desc', 'Determine if this document is mandatory for employees.')}</p>
                                            </div>
                                            <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                                <span className={`text-[10px] font-black uppercase tracking-wider ${formData.is_required ? 'text-amber-500' : 'text-slate-400'}`}>
                                                    {formData.is_required ? t('yes_label') : t('no_label')}
                                                </span>
                                                <Switch
                                        ...
                                        <div className="flex items-center justify-between p-2">
                                            <div className="space-y-0.5">
                                                ...
                                                <p className="text-xs text-slate-500">{t('document_type_status_desc', 'Determine if this document type is currently active.')}</p>
                                            </div>
                                            <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="shrink-0 flex items-center justify-between px-6 py-5 border-t border-slate-100 dark:border-slate-800 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-b-xl">
                            ...
                            <div className="flex gap-3 w-full sm:w-auto">
                                ...
                                >
                                    ...
                                    {isSaving ? t('saving_label') : (editingDocumentType ? t('save_changes_btn_label') : t('create_document_type_btn'))}
                                </Button>
                            </div>
                        </div>
                    </div>
